...
class EnhancedSoilGenerator(keras.utils.Sequence):
    def __init__(self, df, csv_features, batch_size=32, target_size=(IMAGE_SHAPE[0], IMAGE_SHAPE[1]), shuffle=True, **kwargs):
...
def prepare_datasets(df, csv_features, test_size=0.2):
    ...
    train_gen = EnhancedSoilGenerator(train_df, csv_features, batch_size=BATCH_SIZE, shuffle=True)
    test_gen = EnhancedSoilGenerator(test_df, csv_features, batch_size=BATCH_SIZE, shuffle=False)
...
# ---------------- Train / Eval ----------------
def train_enhanced_model(model, train_gen, test_gen):
    ...
    if class_weights is not None:
        if 'fertility_cls' in class_weights:
            cw['fertility_cls'] = class_weights['fertility_cls']
        if 'land_use' in class_weights:
            cw['land_use'] = class_weights['land_use']
    # Keras expects class_weight = dict mapping output_name -> dict(class_label -> weight)
    # cw = {}
    # if 'fertility_cls' in class_weights:
    #     cw['fertility_cls'] = class_weights['fertility_cls']
    # if 'land_use' in class_weights:
    ...
    history = model.fit(
        ...
        # class_weight={
        #     'fertility_cls': class_weights['fertility_cls'],
        #     'land_use': class_weights['land_use']
        # }
        workers=4,
...
if __name__ == "__main__":
    ...
    # Train
    history = train_enhanced_model(model, train_gen, test_gen)
