...
class EnhancedSoilGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for i, (_, row) in enumerate(batch_df.iterrows()):
            ...
            # X_csv[i] = np.array(csv_values, dtype=np.float32)
        ...
        return [X_img, X_csv], {'fertility_reg': y_reg, 'fertility_cls': y_fert, 'land_use': y_land}
...
def compute_class_weights(df, fertility_encoder, land_use_encoder):
    ...
    fertility_classes, fertility_counts = np.unique(df['fertility_class_encoded'], return_counts=True)
    land_use_classes, land_use_counts = np.unique(df['land_use_encoded'], return_counts=True)
    
    # Calculate weights inversely proportional to class frequencies
    fertility_weights = len(df) / (len(fertility_classes) * fertility_counts)
    land_use_weights = len(df) / (len(land_use_classes) * land_use_counts)
    
    # Normalize weights
    fertility_weights = fertility_weights / np.sum(fertility_weights) * len(fertility_classes)
    land_use_weights = land_use_weights / np.sum(land_use_weights) * len(land_use_classes)

    return {
        'fertility_cls': dict(zip(fertility_classes, fertility_weights)),
        'land_use': dict(zip(land_use_classes, land_use_weights))
    }
...
def get_sample_weights(generator, weight_dict, target_name):
        ...
        for i in range(len(generator)):
            ...
            if len(target_values) != (batch_end - batch_start):
                print(f"Warning: Batch size mismatch at batch {i}")
                continue
            
            weights[batch_start:batch_end] = [weight_dict.get(c, 1.0) for c in target_values]
        return weights
...
def train_enhanced_model(model, train_gen, test_gen, class_weights):
    ...
    sample_weights = {
        'fertility_cls': get_sample_weights(train_gen, class_weights['fertility_cls'], 'fertility_cls'),
        'land_use': get_sample_weights(train_gen, class_weights['land_use'], 'land_use')
    }
...
if __name__ == "__main__":
    ...
    for i in range(min(3, len(train_gen))):  # Check first 3 batches
        X, y = train_gen[i]
        print(f"\nBatch {i}:")
        print(f"Images shape: {X[0].shape}")
        print(f"CSV features shape: {X[1].shape}")
